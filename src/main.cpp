...
#if defined(NDEBUG)
# error "POcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2740d32388ac9e7eb00f8151674e138be8faefbbbf0ec2f5401a24a30f542fd3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // POcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "POcoin Signed Message:\n";
...
{
    // POcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 262800); // POcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60; // POcoin: 60 seconds
static const int64 nTargetSpacing = 60; // POcoin: 60 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // POcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // POcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2740d32388ac9e7eb00f8151674e138be8faefbbbf0ec2f5401a24a30f542fd3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "19/dec/16 Dit weten we over de aanslag op de kerstmarkt in Berlijn (nu punt nl)";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040110480fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1482437540;
        ...
        assert(block.hashMerkleRoot == uint256("0x78e078cbd852023e145a4616f5efddb29627205ea89a2e8aeda0b2e6feb757b5"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // POcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("POcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("POcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static POcoinMiner(CWallet *pwallet)
{
    printf("POcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pocoin-miner");
    ...
    try { loop {
        ...
        printf("Running POcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&POcoinMiner, pwallet));
}
